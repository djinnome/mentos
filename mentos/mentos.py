...
def generate_metabolite_report( log_c, forward_rate, backward_rate, S, metabolites, internal_mets, rxns, fullS, mu0,     T = 298.15,     V = 1e-15,     R = 8.3144598/1000.0  ) :
    ...
    mets['fullS*forward_rate'] = pd.DataFrame(np.dot(fullS,forward_rate), index= mets)
    mets['fullS*backward_rate'] = pd.DataFrame(np.dot(fullS,backward_rate), index=mets)
    mets['fullS*net_flux'] = pd.DataFrame(np.dot(fullS,net_flux), index=mets)
    mets['fullS*net_likelihood'] = pd.DataFrame(np.dot(fullS,forward_likelihood - backward_likelihood), index=mets)
    mets['fullS*net_probabilities'] = pd.DataFrame(np.dot(fullS, net_probabilities), index=mets)
